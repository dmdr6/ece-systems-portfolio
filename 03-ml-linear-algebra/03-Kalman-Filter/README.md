...
$$\dot{x}_k = A x_{k-1} + B u_{k-1} + w_{k-1}, \quad x_k \in \mathbb{R}^n, \, u_k \in \mathbb{R}^m$$
...
Let $\hat{x}_k^-$ denote the prior (predicted) state estimate, $P_k^-$ denote the prior error covariance, and $e_k^- = x_k - \hat{x}_k^-$ denote the prior estimation error. Assuming the linear state estimate update:
...
$$P_k^+ = \mathbb{E}\left[ \Big( (I - K_k H) e_k^- - K_k v_k \Big) \Big( (I - K_k H) e_k^- - K_k v_k \Big)^T \right]$$
...
$$J(K_k) = \mathbb{E}\left[ \| e_k^+ \|_2^2 \right] = \mathbb{E}\left[ (e_k^+)^T e_k^+ \right] = \text{tr}\left( \mathbb{E}[e_k^+ (e_k^+)^T] \right) = \text{tr}(P_k^+)$$
...
$$\frac{\partial \text{tr}(P_k^+)}{\partial K_k} = - (H P_k^-)^T - P_k^- H^T + 2 K_k \left( H P_k^- H^T + R \right) = -2 P_k^- H^T + 2 K_k \left( H P_k^- H^T + R \right)$$
...
Substitute this identity into the third and fourth terms of the expanded posterior covariance formula for $P_k^+$:
